...
| 11 | [Freeze: decisions, ADR, tag](tasks/task_11.md) | 02, 04, 05, 06, 09, 10 | you | done |
...
Tag: `platform-foundation-freeze-2026-08-27`.

- FR-012: single-writer long text with explicit takeover.
- FR-013: 60 MB per synchronized campaign.
- Product decisions recorded in [freeze-decisions.md](freeze-decisions.md).
- `main` requires the strict `gates` status check.
- CODEOWNERS uses `@Yggdrae` for every track until more contributors join.
- Spike code deleted after GO findings were preserved in [spike-findings.md](spike-findings.md).
